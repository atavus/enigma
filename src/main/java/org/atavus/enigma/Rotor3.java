...
public class Rotor3 implements Rotor, Wiring, Cloneable {
    ...
    private int[][][] forwardmap = new int[26][26][26];
    private int[][][] reversemap = new int[26][26][26];
    ...
    public Rotor3(final char[] wiring) {
        ...
        for (int pin = 0; pin < 26; pin++) {
            for (int ring = 0; ring < 26; ring++) {
                for (int ofs = 0; ofs < 26; ofs++) {
                    forwardmap[pin][ring][ofs] = (forward[(pin + 52 - ring + ofs) % 26] + 52 - ofs + ring) % 26;
                    reversemap[pin][ring][ofs] = (reverse[(pin + 52 - ring + ofs) % 26] + 52 - ofs + ring) % 26;
                }
            }
        }
    }
    ...
    public void forward(final int pin) {
        int out = forwardmap[pin][ring][offset];
        if (debug)
    ...
    public void reverse(final int pin) {
        int out = reversemap[pin][ring][offset];
        if (debug)
